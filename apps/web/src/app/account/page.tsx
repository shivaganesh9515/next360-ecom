      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <h2 className="font-display text-xl font-bold text-slate-800">Recent Orders</h2>
            ...
            {recentOrders.map(order => (
              <div key={order.id} className="flex justify-between items-center py-3 border-b border-slate-50 last:border-0">
                ...
                <div className="text-right flex flex-col items-end">
                  <p className="font-bold text-slate-800 text-sm mb-1">{formatPrice(order.total)}</p>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm space-y-4">
          <div className="flex items-center justify-between mb-2">
            <h2 className="font-display text-xl font-bold text-slate-800">My Subscriptions</h2>
          ...
          {activeSubs.length > 0 ? (
            ...
            <div className="p-6 text-center border-2 border-dashed border-slate-100 rounded-2xl bg-slate-50">
              <RefreshCw className="mx-auto text-slate-300 mb-2" size={24} />
              <p className="text-sm font-bold text-slate-500 mb-2">No active subscriptions</p>
        ...
        <div className="bg-gradient-to-br from-cream to-white rounded-2xl border border-primary/10 p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <Link href="/account/health-goals" className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-primary transition-colors">Edit Goals</Link>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            {(user?.healthGoals || []).length > 0 ? (
              user?.healthGoals.map(goal => (
                <span key={goal} className="px-3 py-1.5 bg-white border border-slate-100 rounded-lg text-xs font-bold text-slate-700 capitalize shadow-sm">
                  🎯 {goal}
                </span>
              ))
            ) : (
              <p className="text-xs font-medium text-slate-400">Set goals to personalize findings</p>
            )}
          </div>
        </div>
      </div>
...
function QuickAction({ href, icon: Icon, label }: { href: string, icon: any, label: string }) {
  return (
    <Link href={href} className="p-4 bg-white rounded-2xl border border-slate-100 hover:border-primary/20 hover:shadow-lg hover:shadow-primary/5 transition-all group flex items-center justify-between">
      <div className="flex items-center gap-3">
        <div className="w-8 h-8 rounded-lg bg-slate-50 flex items-center justify-center text-slate-500 group-hover:bg-primary/5 group-hover:text-primary transition-colors">
          <Icon size={16} />
        </div>
