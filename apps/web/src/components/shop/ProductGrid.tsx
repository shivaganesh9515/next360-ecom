...
const ProductListSkeleton = () => (
  <div className="flex gap-6 p-4 rounded-3xl border border-slate-100 bg-white">
      ...
      {products.map((product) => (
        ...
        >
          {viewMode === 'grid' ? (
            ...
            <div className="flex flex-col sm:flex-row gap-6 p-4 rounded-3xl border border-slate-100 bg-white hover:shadow-xl hover:shadow-primary/5 transition-all group">
              <div className="relative w-full sm:w-[180px] aspect-square rounded-2xl overflow-hidden bg-cream shrink-0">
                <img 
              ...
              <div className="flex-1 flex flex-col justify-between py-2">
                <div>
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <span className="text-sm font-medium text-slate-400 capitalize">{product.category?.name}</span>
                ...
                <div className="flex items-center justify-between mt-6">
                  <div className="flex items-baseline gap-2">
                    ...
                    {product.originalPrice > product.price && (
                      <span className="text-sm text-slate-400 line-through">₹{product.originalPrice / 100}</span>
                    )}
                  </div>
                  <button className="px-6 py-3 bg-primary text-white font-bold rounded-2xl shadow-lg shadow-primary/20 hover:scale-105 active:scale-95 transition-all">
                    Add to Cart
                  </button>
                </div>
              </div>
