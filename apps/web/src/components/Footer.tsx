...
const Footer = () => {
    ...
    const linkSections = [
        {
            title: "MARKETPLACE",
            links: [
                { text: "Organic Grains", path: '/shop', icon: null },
                { text: "Cold-Pressed Oils", path: '/shop', icon: null },
                { text: "Heritage Spices", path: '/shop', icon: null },
                { text: "Superfoods", path: '/shop', icon: null },
            ]
        },
        {
            title: "PLATFORM",
            links: [
                { text: "About Purity", path: '/', icon: null },
                { text: "Farm Traceability", path: '/', icon: null },
                { text: "Elite Partner Program", path: '/pricing', icon: null },
                { text: "Vendor Enrollment", path: '/create-store', icon: null },
            ]
        },
        {
            title: "SUPPORT",
            links: [
                { text: "+91 98765 43210", path: '/', icon: MailIcon },
                { text: "hello@next360.in", path: '/', icon: PhoneIcon },
                { text: "Hyderabad, Telangana", path: '/', icon: MapPinIcon }
            ]
        }
    ];
    ...
    return (
        <footer className="mx-6 bg-white py-16">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-border/50 text-slate-500">
                    <div>
                        <Link href="/" className="flex items-center gap-1 group">
                           <span className="text-3xl font-display font-black tracking-tighter text-text">Next360<span className="text-secondary text-4xl leading-none">.</span></span>
                        </Link>
                        <p className="max-w-[410px] mt-6 text-sm text-muted font-medium leading-relaxed">Welcome to Next360, India&apos;s premier platform for 100% verified organic produce. We connect you directly to NPOP-certified farms, ensuring total transparency from soil to soul.</p>
                        <div className="flex items-center gap-3 mt-8">
                            {socialIcons.map((item, i) => (
                                <Link href={item.link} key={i} className="flex items-center justify-center w-10 h-10 bg-cream hover:bg-secondary/10 hover:scale-105 transition-all rounded-xl">
                                    <item.icon />
                                </Link>
                            ))}
                        </div>
                    ...
                    <div className="flex flex-wrap justify-between w-full md:w-[50%] gap-8 text-sm ">
                        {linkSections.map((section, index) => (
                            <div key={index} className="min-w-[140px]">
                                <h3 className="font-black text-[10px] uppercase tracking-widest text-text mb-6">{section.title}</h3>
                                <ul className="space-y-4">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-center gap-2">
                                            {link.icon && <link.icon />}
                                            <Link href={link.path} className="text-muted hover:text-primary transition-colors font-medium">{link.text}</Link>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                ...
                <div className="py-8 flex flex-col md:flex-row items-center justify-between gap-4">
                    <p className="text-xs text-muted font-medium">
                        Copyright 2026 © Next360. All Rights Reserved. Purely Organic.
                    </p>
                    <div className="flex items-center gap-6">
                       <Link href="/" className="text-[10px] font-bold uppercase tracking-wider text-muted hover:text-text">Privacy</Link>
                       <Link href="/" className="text-[10px] font-bold uppercase tracking-wider text-muted hover:text-text">Terms</Link>
                       <Link href="/" className="text-[10px] font-bold uppercase tracking-wider text-muted hover:text-text">NPOP Guidelines</Link>
                    </div>
                </div>
            </div>
        </footer>
